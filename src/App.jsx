...
import { Archive, Plus, Pencil, Printer, DatabaseBackup, Search, Building2, Trash2, FileSpreadsheet, RefreshCw, Eye, FolderCog, CalendarDays, Clock3, Save } from 'lucide-react';
...
export default function App(){
 ...
 async function printTable(){try{const r=await api.listDeeds({...filters,page:1,pageSize:5000});setTableReport({title:'تقرير الصكوك والعقارات',subtitle:'جميع السجلات المطابقة لمعايير البحث والفلاتر الحالية',items:r.items||[]})}catch{setToast('تعذر تجهيز تقرير الطباعة')}}
 async function printOwner(){if(!selected?.owner_name){setToast('اختر صكًا أولاً لتحديد المالك');return}try{const r=await api.listDeeds({...initialFilters,owner:selected.owner_name,page:1,pageSize:5000});setTableReport({title:'تقرير صكوك المالك',subtitle:`جميع الصكوك المسجلة باسم ${selected.owner_name}`,owner:selected.owner_name,items:r.items||[]})}catch{setToast('تعذر تجهيز تقرير المالك')}}
 ...
 return <div className="app-shell">
  ...
  <footer className="statusbar"><div><span className="status-dot"/> قاعدة البيانات المحلية جاهزة</div><div>{loading?'جارٍ تحديث البيانات...':`${result.total} صك في النتائج الحالية`}</div><div>SQLite • تخزين محلي • V0.2.6</div></footer>
  <DeedForm open={formOpen} deed={editing} onClose={()=>setFormOpen(false)} onSave={saveDeed}/>
  <AttachmentViewer attachment={viewer} onClose={()=>setViewer(null)}/><PrintPreview deed={printPreview?selected:null} onClose={()=>setPrintPreview(false)}/><TablePrintPreview report={tableReport} onClose={()=>setTableReport(null)}/>
  <StorageSettings open={storageOpen} onClose={()=>setStorageOpen(false)} onToast={setToast} onChanged={async()=>{setSelected(null);await loadOptions();await load()}}/><SmartImport open={importOpen} onClose={()=>setImportOpen(false)} onDone={importDone} onToast={setToast}/><UpdateSettings open={updatesOpen} onClose={()=>setUpdatesOpen(false)} onToast={setToast} initialStatus={updateStatus}/>
